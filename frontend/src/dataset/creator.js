...
export default class DatasetCreator extends Component {
    state = {
        loading: false,
        createdTag: null,
        tag: '',
    ...
    createDataset = async () => {
        ...
        try {
            ...
            await API.createManagedDataset(
                ...
                touchedDatasets,
                !triggered,
                frequency ? `${frequency}.minutes` : null,
    ...
    render() {
        const {createdTag, tagValid, loading, tag, manual, frequency, unbounded, retention, owner} = this.state
        const {step} = this.state
        ...
        return <Segment className='dataset-creator' textAlign='center'>
            {createdTag ?
                ...
                <Form onSubmit={() => step === 2 ? this.createDataset() : this.nextPage()}>
                    ...
                    <div className={'dataset-create-step ' + (step === 1 ? '' : 'hidden')}>
                        <EvaluationStrategy touchedDatasets={touchedDatasets} onSelect={a => this.setState(a)}/>
                        <MaterializationStrategy onSelect={a => this.setState(a)}/>
                    </div>

                    <div className={'dataset-create-step ' + (step === 2 ? '' : 'hidden')}>
                        <SegmentTTL onSelect={a => this.setState(a)}/>
                        <AvailabilityClass touchedDatasets={touchedDatasets} onSelect={a => this.setState(a)}/>
                    </div>
                    ...
                    <Segment basic>
                        ...
                        {step === 2 ?
                            <Button type='submit' disabled={!(infoValid && matValid && ttlValid)} positive loading={loading}>Create</Button>
                            : null
                        }
                    </Segment>
                </Form>
