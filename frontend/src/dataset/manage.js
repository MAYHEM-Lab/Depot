...
export default function DatasetManage() {
    ...
    const originalRetention = util.parseDuration(dataset.retention).minutes().toString()
    const originalSchedule = util.parseDuration(dataset.schedule).minutes().toString()

    const retentionInput = useRef(null)
    const [retention, setRetention] = useState(dataset.retention && originalRetention)
    const [retentionEnabled, setRetentionEnabled] = useState(!!dataset.retention)

    const scheduleInput = useRef(null)
    const [schedule, setSchedule] = useState(dataset.schedule && originalSchedule)
    ...
    const changed = parseInt(originalSchedule) !== parseInt(schedule) || parseInt(originalRetention) !== parseInt(retention) || dataset.description !== description || dataset.visibility !== visibility
